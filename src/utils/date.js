export const DateFormat = (originalDateString) => {
  ...
  const formattedDate = new Date(originalDateString).toLocaleDateString('id-ID', options)
...
export const TimeFormat = (originalTimeString) => {
  ...
  const formattedTime = new Date(originalTimeString).toLocaleTimeString('id-ID', options)
