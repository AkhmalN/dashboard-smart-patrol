...
import Detailaktivitas from './detailaktivitas.vue'
...
onMounted(() => store.fetchAktivitas())
</script>
<template>
  <CCard>
    <CCardHeader>
      ...
      <CTable id="my-table">
        <CTableHead color="light">
          <CTableRow>
            ...
            <CTableHeaderCell scope="col">Tangal</CTableHeaderCell>
        ...
        <CTableBody>
          ...
          <template v-else-if="store.aktivitasData.length < 1">
            <CTableRow>
              <CTableDataCell colspan="4" class="d-flex align-items-center gap-2">
          ...
          <template v-else>
            <CTableRow v-for="(data, index) in store.paginatedData" :key="data._id">
              ...
              <CTableDataCell>{{ data.pos_aktivitas }}</CTableDataCell>
              <CTableDataCell>{{ data.instansi_aktivitas }}</CTableDataCell>
              <CTableDataCell>{{ DateFormat(data.createdAt) }}</CTableDataCell>
              <CTableDataCell>{{ TimeFormat(data.createdAt) }}</CTableDataCell>
              <!-- <CTableDataCell>
                <CRow>
                  <CCol sm="auto">
                    <Detailaktivitas :data="data._id" />
                  </CCol>
                </CRow>
              </CTableDataCell> -->
            </CTableRow>
