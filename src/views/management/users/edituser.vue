...
async function handleSubmit() {
  ...
  try {
    const response = await store.fetchEditUser(data, editData)
    if (response.status === 200) {
      visibleModal.value = false
      toggleModal(false)
      Swal.fire({
        title: 'Success!',
        text: 'Berhasil mengedit data user',
        icon: 'success',
      })
    ...
    Swal.fire({
      title: 'Error!',
      text: 'Terjadi kesalahan saat mengedit data user',
      icon: 'error',
    })
  }
}
...
<template>
  <CButton color="warning" @click="toggleModal(true)">
    <CIcon :icon="cilPencil" class="text-light" />
  </CButton>
      ...
      <template v-else>
        <CForm @submit.prevent="handleSubmit">
          ...
          <CModalFooter>
            ...
            <CButton type="submit" color="primary" :disabled="store.loadingUpdate">
              <CSpinner color="light" class="text-sm" size="sm" v-show="store.loadingUpdate" />
